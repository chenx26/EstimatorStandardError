...
SoR.IF=function(data,rf=0){
  ...
  tmp=-SoR.hat/2/sigma.minus.hat^2*(data-mu.hat)^2*(data<=mu.hat)+
    (1/sigma.minus.hat+SoR.hat*mu1.minus.hat/sigma.minus.hat^2)*(data-mu.hat)+
    SoR.hat/2
  return(tmp)
}
...
#' @examples
#' STARR.IF(rnorm(10))
STARR.IF=function(data,alpha=0.1){
  mu.hat=mean(data)
  sigma.hat=mean((data-mu.hat)^2)
  VaR.hat=-quantile(data,alpha)
  ES.hat=-mean(data[data<=-VaR.hat])
  STARR.hat=(mu.hat-rf)/ES.hat
  tmp=(data-mu.hat)/ES.hat-STARR.hat/ES.hat*(1/alpha*((-data-VaR.hat)*(data<=qa))+VaR.hat-ES.hat)
  return(tmp)
}
